...
class Binarize(torch.autograd.Function):
    @staticmethod
    def forward(context, probs,k=2):
        # binarized = (probs == torch.max(probs, dim=1, keepdim=True)[0]).float()
        binarized = torch.zeros(probs.shape)
        for i, l in enumerate(probs.topk(k=2, dim=1)[1]):
            binarized[i, l] = 1
...
class Flgc2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        super().__init__()
        self.in_channels_in_group_assignment_map = nn.Parameter(torch.Tensor(in_channels, groups)) #[3,4]
        nn.init.normal_(self.in_channels_in_group_assignment_map)
        self.out_channels_in_group_assignment_map = nn.Parameter(torch.Tensor(out_channels, groups)) # [16.4]
    ...
    def forward(self, x):
        map = torch.mm(self.binarize(torch.softmax(self.out_channels_in_group_assignment_map, dim=1)),
                       torch.t(self.binarize(torch.softmax(self.in_channels_in_group_assignment_map, dim=1)))) # torch.t 为转置
        return nn.functional.conv2d(x, self.conv.weight * map[:, :, None, None], self.conv.bias,
                                    self.conv.stride, self.conv.padding, self.conv.dilation) # 分组卷积输出X(W*(TS^T))+b
